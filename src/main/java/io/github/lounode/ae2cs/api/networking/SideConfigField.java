...
public record SideConfigField(EnumMap<Direction, SidePolicy> sidePolicies,
    ...
    private static final Codec<EnumMap<Direction, SidePolicy>> SIDE_POLICIES_CODEC = Codec.unboundedMap(Direction.CODEC, SidePolicy.CODEC)
            .xmap(map -> {
    ...
    public static final Codec<SideConfigField> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.BOOL.optionalFieldOf("auto_export", false).forGetter(SideConfigField::autoExport)).apply(instance, SideConfigField::new));

    public static final StreamCodec<RegistryFriendlyByteBuf, SideConfigField> STREAM_CODEC = ByteBufCodecs.fromCodecWithRegistries(CODEC);
