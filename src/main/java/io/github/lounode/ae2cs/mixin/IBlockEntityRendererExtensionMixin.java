...
{
    @ModifyReturnValue(
            method = "getRenderBoundingBox()Lnet/minecraft/world/phys/AABB;",
            at = @At("RETURN")
    )
    private AABB aecs$getRenderBoundingBox(AABB original)
    {
        if (!(this instanceof CableBusBlockEntity te))
        {
            return original;
        }
