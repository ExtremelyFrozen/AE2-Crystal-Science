...
public class AECSDataComponents {
    ...
    public static final DeferredRegister<DataComponentType<?>> DATA_COMPONENTS = DeferredRegister.create(Registries.DATA_COMPONENT_TYPE, AECSConstants.MODID);
    ...
    // 生长进度
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<Integer>> GROW_PROCESS = register("grow_process", b -> b
            .persistent(Codec.INT)
            .networkSynchronized(ByteBufCodecs.VAR_INT)
            .cacheEncoding());

    // 频段名称-给内存卡记录用
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<MemoryCardBandInfo>> MEMORY_CARD_BAND_INFO = register("band_name", b -> b
            .persistent(MemoryCardBandInfo.CODEC)
            .networkSynchronized(MemoryCardBandInfo.STREAM_CODEC)
            .cacheEncoding());

    // 记录作为接收端时期望的频段数量
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<Integer>> MEMORY_CARD_BROADCASTER_RECEIVER_EXPECT_CHANNELS = register("meory_card_broadcaster_receiver_expect_channels", b -> b
            .persistent(Codec.INT)
            .networkSynchronized(ByteBufCodecs.VAR_INT)
            .cacheEncoding());

    // 记录发信器信息坐标位置
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<GlobalPos>> ENDER_EMITTER_POS = register("ender_emitter_pos", b -> b
     ...
     * 用于存储谐振样板信息
     */
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<EncodedResonatingPattern>> ENCODED_RESONATING_PATTERN = register("encoded_resonating_pattern", b -> b
     ...
     * 用于记录当前谐振样板的目标资源索引
     */
    public static final DeferredHolder<DataComponentType<?>, DataComponentType<Integer>> RESONATING_PATTERN_SELECTED_INPUT = register("resonating_pattern_selected_input", b -> b
            .persistent(Codec.INT)
            .networkSynchronized(ByteBufCodecs.VAR_INT)
            .cacheEncoding());
