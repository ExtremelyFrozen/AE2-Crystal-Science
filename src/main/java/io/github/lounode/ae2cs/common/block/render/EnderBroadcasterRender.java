...
public class EnderBroadcasterRender implements BlockEntityRenderer<EnderBroadcasterBlockEntity, EnderBroadcasterRender.State>
{
    // 缓存参数，避免每帧重复生成
    ...
    public void extractRenderState(
            EnderBroadcasterBlockEntity be,
            State state,
            float partialTick,
            Vec3 cameraPosition,
    ...
    )
    {
        ...
        if (be.isActiveForClient())
        {
            if (be.isAsSenderForClient())
                state.coreModel = modelManager.getStandaloneModel(AECSAdditionalModels.BROADCASTER_SENDER_CORE_MODEL);
            else
                state.coreModel = modelManager.getStandaloneModel(AECSAdditionalModels.BROADCASTER_RECEIVER_CORE_MODEL);
        }
        else
        {
            state.coreModel = modelManager.getStandaloneModel(AECSAdditionalModels.BROADCASTER_OFF_CORE);
        }
        ...
        state.scale = scale;
        state.rotation.set(q);
    }

    @Override
    public void submit(State state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera)
    {
        if (state.coreModel == null)
        {
            return;
        }
    ...
    {
        private BlockStateModelPart coreModel;
