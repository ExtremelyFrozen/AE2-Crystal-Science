...
public record EncodedResonatingPattern(
    ...
    public static final Codec<Target> TARGET_CODEC = RecordCodecBuilder.create(b -> b.group(
            ...
            Direction.CODEC.fieldOf("face").forGetter(Target::face)).apply(b, Target::new));

    public static final Codec<Optional<Target>> OPTIONAL_TARGET_CODEC = Codec.either(TARGET_CODEC, Codec.unit(Unit.INSTANCE))
            .xmap(
                    e -> e.left().map(Optional::of).orElse(Optional.empty()),
                    opt -> opt.<Either<Target, Unit>>map(
                            Either::left).orElseGet(() -> Either.right(Unit.INSTANCE)));
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, Optional<Target>> OPTIONAL_TARGET_STREAM_CODEC = ByteBufCodecs.optional(TARGET_STREAM_CODEC);
