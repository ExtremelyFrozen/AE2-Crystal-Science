...
- 可主动从周围容器回收当前已安装样板所对应的产物，而不依赖外部管道
- 可为普通处理样板提供一套默认的投放目标
...
这样同一台设备上可以同时兼容“精确标记样板”和“统一默认规则”两种工作流。
...
谐振样板供应器支持主动回收产物，但它不会无差别抽取附近容器中的一切内容。

- 仅会回收**当前已安装样板产物**对应的内容
- 适合将加工完成后的结果重新送回供应器返回仓
- 不会把无关物品一并抽走

这样不用增加额外物流就能，让处理结果更稳定地回到 AE 网络中。
