---
navigation:
  parent: index.md
  title: 初级样板供应器
  icon: ae2cs:simple_pattern_provider
  position: 160
item_ids:
  - ae2cs:simple_pattern_provider
  - ae2cs:simple_pattern_provider_part
  - ae2cs:simple_circuit_print
  - ae2cs:simple_processor
  - ae2cs:pattern_provider_upgrade

---

# 初级样板供应器

<Row>
  <BlockImage id="simple_pattern_provider" scale="4" />
  <ItemImage id="simple_pattern_provider_part" scale="4" />
  <ItemImage id="pattern_provider_upgrade" scale="4" />
</Row>

初级样板供应器是样板供应器的简化版本，材料配方都更简单，可容纳的样板槽位也更少，只有五个。不过为了制作它，你还是需要两个<ItemLink id="simple_processor" />。
不过它们的配方也十分简单，甚至不需要压印模板。并且能通过<ItemLink id="pattern_provider_upgrade" />升级为普通版本的样板供应器

<Row>
    <Recipe id="ae2cs:inscriber/simple_circuit_print" /> 
    <Recipe id="ae2cs:inscriber/simple_processor" /> 
    <Recipe id="ae2cs:craft/shaped/simple_pattern_provider" /> 
</Row>

